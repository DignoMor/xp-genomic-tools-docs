...
ExogenousSequenceTools <subcommand> --help
```

!!! note "Spelling"
    The command name is **ExogenousSequenceTools** (with that spelling).
...
```bash
ExogenousSequenceTools motif_search --help
```

Track reduction:

```bash
ExogenousSequenceTools track_dim_reduction max --help
```
