...
1. Genomic position of the reported strand-oriented 5-prime base uses
   `linear(coord) = coord - 1` when `coord > 0`, else `coord`. Strand `+`
   uses `tss + linear(coord)`; strand `-` uses `tss - linear(coord)` so
   negative coordinates are transcription-upstream.
